...
def init_db(name,model):
    ...
    if model == 0:
        try:
            #创建数据库。并保存到complete.db
            conn = sqlite3.connect(folder+'/complete.db')
            #创建的包含了字段id http_url time backup_1-4
            sql_creat_table = '''
        ...
        try:
            conn = sqlite3.connect(folder+'/complete.db')
            #创建的包含了字段id http_url time backup_1
            sql_creat_table = '''
                        create table if not exists info(
                        id integer primary key autoincrement,
                        http_url varchar(256) DEFAULT NULL,
...
def engine_db(dbname,complete_queue,smart_queue,model):
    #print (dbname,complete_queue,smart_queue,model)
